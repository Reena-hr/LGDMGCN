...
def main(config, adj_mat):
    ...
    # build model architecture, then print to
    ...
    if config["arch"]["type"] == "LSTM":
        model_type = "lstm"
        supports = None
    elif config["arch"]["type"] == "STSeq2Seq":
        model_type = "stseq2seq"
        supports = None
    else:
        model_type = "seq2seq"
...
if __name__ == '__main__':
    ...
    # # Single Graph
    # adj_mat = pd.read_csv('Data/Adj_his.csv')

    # Multi Graph (global)
    # Adj_his = pd.read_csv('Data/Adj_his.csv')
    # Adj_poi = pd.read_csv('Data/Adj_poi.csv')
    # adj_mat = pd.read_csv('Data/Adj_global.csv')
    
    adj_mat = np.load('Data/Adj_global.npy')

    args = argparse.ArgumentParser(description='Seq2Seq')
