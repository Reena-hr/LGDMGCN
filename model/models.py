...
class STSeq2Seq(nn.Module):
    ...
    def forward(self, supports, source, target, teacher_forcing_ratio, batch_id, batch_size, dy_supports, tinterval, order2time_map):
        ...
        encoder_outputs, hidden = self.encoder(supports, source, batch_id, batch_size, dy_supports, tinterval, order2time_map)
        ...
        outputs, preds = self.decoder( inputs=target, external_inputs=None,attention_states=encoder_outputs,
                               external_flag=False, output_size=64)
        return preds  # (seq_length, batch_size, num_nodes*output_dim)  (12, 64, 207*1)
